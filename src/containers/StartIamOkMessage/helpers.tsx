...
export const getIamOkGroups = (groups: Group[]): Group[] => {
  return filterGroupsNoCoAlertNoCO(groups).filter(group =>
...
export const useGetMembers = (props: {
  ...
  const { id, setUserIds, withoutCurrentUserId } = props;
  ...
  const init = async () => {
    if (!id) return;
    setIsLoading(true);
    const _members = await getGroupMembers({ id: id });
    const members = withoutCurrentUserId
      ? _members.filter(member => member.userID !== withoutCurrentUserId)
      : _members;
