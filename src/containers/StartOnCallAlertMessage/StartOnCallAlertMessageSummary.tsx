...
import { SIcon, SInput, SItem, SSimpleText, SLine } from './styles';
...
import { fetchOnCallAlertList } from '../OnCallAlertList/onCallAlertSlice/actionCreators';
import { sendOnCallAlertMessage } from '../../apis/onCallAlertApi';
...
export const SSummaryTab = styled(SummaryTab)`
...
export const StartOnCallAlertMessageSummary = () => {
  ...
  const {
    ...
    setSubject,
    onFinish,
    onCallAlertMessageText: text,
    setOnCallAlertMessageText: setText,
    userIds,
    setUserIds,
  } = useStartOnCallAlertMessageCtx();
  ...
  const handleMessageSend = async (text: string) => {
    try {
      setIsLoading(true);
      const { id } = await sendOnCallAlertMessage({
        groupId: groupIds!,
        text: text,
  ...
  const goToSelectGroupList = () => {
    navigation('/onCallAlert');
  };
  const gotToOnCallAlertList = () => {
    navigation('/onCallAlert');
  };
  const gotToOnCallAler = (id: number) => {
    navigation(`/oncall/${id}`);
  };
  ...
  return (
    <SContainer>
      ...
      <SItem className="input">
        ...
        <SIcon src={Pencil} alt="" onClick={highlightText} className="pencil" />
      </SItem>
